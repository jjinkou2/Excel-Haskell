...
servToPair rows s = (s,servToVal rows s)

servToVal ::  [String] -> T.Text -> Value
servToVal rows s  = object [s.= (kpiPairsToVal.rawToPairs $ rows)]
kpiPairsToVal ::  [Pair] -> Value
kpiPairsToVal dict = object.map (uncurry (.=)) $ dict


toServStruct serv rows = ServStruct serv (rawToStruct rows) 

rawToPairs :: [String] -> [Pair] 
rawToPairs rows  = zip kpiNames kpiValues
