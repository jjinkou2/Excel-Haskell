...
printData workSheets sheetName = do 
    ...
    nbMinutes       <- lookupData toDouble 0.0 rowMinutes
    nbCalls         <- lookupData toDouble 0.0 rowCalls
    postGAD         <- lookupData toDouble 0.0 rowPgad
    asr             <- lookupData toDouble 0.0 rowAsr
    ner             <- lookupData toDouble 0.0 rowNer
    attps           <- lookupData toDouble 0.0 rowAttps
    afis            <- lookupData toDouble 0.0 rowAfis 
    mos             <- lookupData toDouble 0.0 rowMos 
    pdd             <- lookupData toDouble 0.0 rowPdd
    csr             <- lookupData toDouble 0.0 rowCsr 
    rtd             <- lookupData toDouble 0.0 rowRtd 
    avail           <- lookupData toDouble 0.0 rowAvail 
    unavail         <- lookupData toDouble 0.0 rowUnAvail 
...
testSplit = coRun $ do --(2.67 secs, 30887528 bytes)
    ...
    printDataSplit rows  
    rows <- rowsFromSheet workSheets "BIC"
    putStrLn "got all datas from BIC"
